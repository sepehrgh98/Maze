...
//Pens
QPen redPen(red,0);
QPen cyanPen(Qt::cyan,0);
QPen yellowPen(yellow,0);
    ...
    , ui(new Ui::MazeGUI)
{
    ...
    //hide main page to show welcome page
    ui->mainPage->setHidden(true);

    //set image to logo boxes
    ...
    //hide control bar in start
    ui->controlBar->setHidden(true);

    //hide step label and step counter in start
    ui->steplabel->setHidden(true);
    ui->stepnumber->setHidden(true);

    //set image to logo box
    ...
    //rowSpinBox
    ui->rowspinBox->setRange(3,60);

    //colSpinBox
    ui->colspinBox->setRange(3,60);
